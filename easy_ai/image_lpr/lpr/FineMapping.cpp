...
namespace pr{
    ...
    FineMapping::FineMapping(ModelLoader *modeloader) {
        Model* model= modeloader->readModel(1);//
...
//            cv::cvtColor(FinedVertical,FinedVertical,cv::COLOR_GRAY2BGR);
    ...
    cv::Mat FineMapping::FineMappingVertical(cv::Mat InputProposal,int sliceNum,int upper,int lower,int windows_size,int paddingUpper,int paddingLower){
...
//
            ...
            std::pair<int, int> A;
            std::pair<int, int> B;
            A = FitLineRansac(line_upper, paddingUpper);
            B = FitLineRansac(line_lower, paddingLower);
            int leftyB = A.first;
...
//        image = cv2.warpPerspective(rgb,mat,(136,36),flags=cv2.INTER_CUBIC)
            ...
            std::vector<cv::Point2d> corners_trans__(4);

...
typedef struct landmark_yolov5_det_s {
	float score;
	int id;
	char label[YOLOV5_MAX_LABEL_LEN];
	float x_start; // normalized value
	...
	float p4_x;
	float p4_y;
} landmark_yolov5_det_t;

typedef struct  landmark_yolov5_result_s {
	landmark_yolov5_det_t detections[YOLOV5_MAX_OUT_NUM];
	int valid_det_count;
} landmark_yolov5_result_s;

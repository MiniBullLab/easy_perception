...
static void * show_vout_thread(void* overlay_thread_param)
{
	int rval = 0;
	uint32_t i = 0;
...
#endif
			ea_display_set_textbox(overlay_ctx.display,
				overlay_ctx.final_plate_list.license_plate[i].text,
				overlay_ctx.x_offset, plate_y, plate_w, plate_h);
	...
	uint32_t draw_plate_num, uint32_t debug_en)
{
	...
	do {
		...
		overlay_ctx.update_cond_flag = 1;
		overlay_ctx.display = ea_display_new(EA_DISPLAY_VOUT,
			EA_DISPLAY_ANALOG_VOUT, EA_DISPLAY_BBOX_TEXTBOX, NULL);

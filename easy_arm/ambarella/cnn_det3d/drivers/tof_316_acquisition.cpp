...
int TOF316Acquisition::get_tof_Z(cv::Mat &depth_z)
{
	int ret = 0;
	if(pthread_id > 0 && run_tof > 0) 
	{
		float* depth_ptr = depth_z.ptr<float>(0);
		pthread_mutex_lock(&tof_buffer.lock);  
		if (tof_buffer.writepos == tof_buffer.readpos)  
		{  
			pthread_cond_wait(&tof_buffer.notempty, &tof_buffer.lock);  
		...
		for(int i = 0; i < MAX_POINT_CLOUD && run_tof > 0; i++)
		{
			*depth_ptr = tof_buffer.buffer_z[tof_buffer.readpos][i];
			depth_ptr++;
		}
		tof_buffer.readpos++;  
		if (tof_buffer.readpos >= TOF_BUFFER_SIZE)  
...
int TOF316Acquisition::get_tof_pc(PointCloud &point_cloud)
{
	int ret = 0;
	int i, j, index;
	float inv_dst = 0;
...
int TOF316Acquisition::dump_z_txt(const std::string &save_path, const cv::Mat &depth_z)
{
	std::ofstream out_file(save_path);
	for (int i = 0; i < depth_z.rows; ++i)
	{
		const float* p = depth_z.ptr<float>(i);
		for (int j = 0; j < depth_z.cols; ++j)
		{
			out_file << p[j] << " ";
		}
		out_file << std::endl;
